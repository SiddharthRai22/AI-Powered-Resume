...
export default function Navbar() {
  ...
  const navStyle = {
    ...
    background: scrolled ? 'rgba(11,13,26,0.92)' : 'rgba(11,13,26,0.7)',
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px', display: 'flex', alignItems: 'center', height: 64 }}>
          ...
          {navLinks.map(({ to, label, icon: Icon }) => (
            <Link
              ...
              style={{
                ...
                color: isActive(to) ? '#818cf8' : 'rgba(241,245,249,0.6)',
                border: isActive(to) ? '1px solid rgba(99,102,241,0.25)' : '1px solid transparent',
                transition: 'all 0.2s',
              }}
              onMouseEnter={e => { if (!isActive(to)) { e.currentTarget.style.color = '#f1f5f9'; e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; }}}
              onMouseLeave={e => { if (!isActive(to)) { e.currentTarget.style.color = 'rgba(241,245,249,0.6)'; e.currentTarget.style.background = 'transparent'; }}}
            >
              <Icon size={14} />
              {label}
            </Link>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginLeft: 'auto' }}>
          {user ? (
            <div id="user-dropdown" style={{ position: 'relative' }}>
              ...
              {dropdownOpen && (
                <div className="animate-fade-in" style={{
                  ...
                  boxShadow: '0 20px 50px rgba(0,0,0,0.5)', zIndex: 200,
                  background: '#151728', border: '1px solid rgba(255,255,255,0.08)',
                  backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)'
                }}>
                  <div style={{ padding: '10px 12px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                    <p style={{ fontSize: 11, color: 'rgba(241,245,249,0.4)', marginBottom: 4, fontWeight: 500 }}>Signed in as</p>
                    <p style={{ fontSize: 13, fontWeight: 600, color: '#f1f5f9', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user.email}</p>
                  </div>
                  <button
            ...
            <>
              <Link to="/login" style={{ fontSize: 13, fontWeight: 500, color: 'rgba(241,245,249,0.65)', textDecoration: 'none', padding: '7px 12px', transition: 'color 0.2s' }}
          ...
          <button
            ...
            style={{ display: 'none', padding: 8, borderRadius: 8, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', color: 'rgba(241,245,249,0.7)' }}
       ...
       {/* Mobile menu */}
      {mobileOpen && (
        <div className="animate-fade-in" style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '12px 20px', display: 'flex', flexDirection: 'column', gap: 4, background: '#0b0d1a' }}>
          {navLinks.map(({ to, label, icon: Icon }) => (
            <Link key={to} to={to} onClick={() => setMobileOpen(false)}
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', borderRadius: 10, fontSize: 14, fontWeight: 500, color: 'rgba(241,245,249,0.75)', textDecoration: 'none' }}
            >
              <Icon size={16} />{label}
            </Link>
          ))}
          {user ? (
            ...
            <div style={{ display: 'flex', gap: 8, paddingTop: 8, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
              <Link to="/login" onClick={() => setMobileOpen(false)} style={{ flex: 1, padding: '10px', textAlign: 'center', fontSize: 14, color: 'rgba(241,245,249,0.7)', textDecoration: 'none', borderRadius: 10 }}>Sign in</Link>
